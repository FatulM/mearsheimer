#!/usr/bin/env python3
"""
Simplify the language of a Persian blog post in plain Persian (Farsi).

This is a post-generation refinement pass: reads the current Persian post from
content/episode-N.md (N>=1), sends it with prompts/simplify-language.md as the
system prompt to the configured LLM (using LLM_MODEL_SIMPLIFY, falling back to
LLM_MODEL — never LLM_MODEL_CRITIQUE), and rewrites content/episode-N.md with
the simplified text.

The pass is language simplification, not summarization: the post keeps its
structure (H1 title and every chapter heading verbatim, with the same
timestamps and titles) and all of its content; only the wording is made
simpler and more natural for Iranian readers.
...
Usage:
    python3 scripts/simplify_language.py 1        # simplify episode 1
"""
...
def read_inputs(n: int) -> tuple[str, str]:
    """Read and return (system_prompt, content) for episode N."""
    content_path = CONTENT_DIR / f"episode-{n}.md"
    prompt_path = PROMPTS_DIR / "simplify-language.md"
    ...
    return prompt_path.read_text("utf-8"), content_path.read_text("utf-8")
...
def generate(
    ...
    """Call the OpenAI-compatible endpoint and return the simplified post."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Simplify the Persian wording of a content post."
    )
    ...
    system_prompt, content = read_inputs(args.episode)
    check_endpoint(base_url, api_key)
    simplified = generate(base_url, api_key, model, system_prompt, content)
    simplified = normalize_content(simplified)

    content_path = CONTENT_DIR / f"episode-{args.episode}.md"
    content_path.write_text(simplified, "utf-8")
