#!/usr/bin/env python3
"""
Fact-check and critique a Persian blog post for an episode, with source citations.

Reads the Persian content from content/episode-N.md and the matching system
prompt (prompts/research-critique-cite.md by default, or
prompts/research-critique-full-cite.md with --full, which also passes the
English transcript from processed/episode-N.md), fact-checks each chapter via
the web_search tool, and writes the result — including inline `[cite: N]`
markers and a numbered URL list — to critique/episode-N-cite.md. It uses
LLM_MODEL_CRITIQUE for the fact-checking, falling back to LLM_MODEL when unset.
...
def read_inputs(n: int, full: bool) -> tuple[str, str]:
    ...
    prompt_name = (
        "research-critique-full-cite.md" if full else "research-critique-cite.md"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Critique a Persian blog post for an episode, with citations."
    )
    ...
    out_path = CRITIQUE_DIR / f"episode-{args.episode}-cite.md"
