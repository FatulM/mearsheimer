...
Translate the title into Persian whenever possible; the original title and outlet name may remain in English; every entry must end with its full URL. Do not substitute other formatting — no `*…*` emphasis, no italics, no bullets, no other separators, no omitted URLs. Keep a blank line between the last body paragraph and the `---`, a blank line between the `---` and the first citation entry, and a blank line after the last citation entry — the whole output must end with a blank line.
