...
Citations are essential to this task: an output without citations is not acceptable — a critique that asserts sourced facts but provides no references is incomplete. You must add at least one `[cite: N]` inline reference in every chapter section that presents research findings, placing it next to the most representative sourced claim in that chapter; a chapter that presents research findings but carries no inline reference is a defect that makes the output failed. Use `[cite: N]` for one source or `[cite: N,M]` (and up to any number of comma-separated entries) for several. The numbers are English digits (0-9), comma-separated with no spaces, and refer to the numbered list in the citations section. Place the reference immediately after the sentence (or clause) it supports, before the final punctuation. Only cite sources you actually consulted — never fabricate a citation. Reference numbers may be reused, and a single claim may carry several references. It is acceptable to leave a chapter without an inline reference only in rare cases where that chapter genuinely has no research-backed claim to hang one on (for example a purely introductory or transitional section); make such cases the exception, not the norm. A citations section with zero `[cite: N]` markers anywhere in the body is a failed output even if the assessment text is otherwise correct. Before finishing, audit your output: confirm research-bearing chapter sections carry inline citations, and only omit them where a chapter truly has nothing to cite.
...
The citations section is mandatory: a valid output must always end with a citations section containing real sources found during research — an output without citations is incorrect regardless of the quality of the assessment. Append the citations section at the very end of the output, separated from the assessment by a blank line and a single horizontal rule (`---`). Do not add any heading above the citations section, and do not use the horizontal rule (`---`) anywhere else in the output. Present the citations as a single-line numbered Markdown list with numbers in English digits (1., 2., ...). Each entry must follow exactly one line in this form, using ` — ` (em-dash) as the only separator and giving a URL for every entry:
`{N}. {Persian title of the source} — {Original title / outlet (English allowed)} — {URL}`
Translate the title into Persian whenever possible; the original title and outlet name may remain in English; every entry must end with its full URL. Do not substitute other formatting — no emphasis of any kind (no `*…*` italics, no `**…**` bold), no bullets, no other separators, no omitted URLs. Only include URLs you actually retrieved while researching — never synthesize a URL from memory, never guess an article ID or slug, never use typo'd domains or generic landing pages; every URL must be one you truly visited. Keep a blank line between the last body paragraph and the `---`, a blank line between the `---` and the first citation entry, and a blank line after the last citation entry — the whole output must end with a blank line.

The citations section must be self-consistent with the body. Every `[cite: N]` reference in the body must map to its numbered entry, so no reference may point to a missing or out-of-range entry — and conversely, every entry in the list must be referenced by at least one `[cite: N]` in the body; a listed entry that nothing cites is a defect that makes the output failed. Number the entries sequentially (1., 2., 3., ...) in the order they are first referenced. Before finishing, verify that (1) every distinct `[cite: N]` number has a matching entry, (2) every entry is referenced at least once, and (3) the chapter `##` heading count and identity match the input exactly.
