...
4. For every episode with a `critique/episode-N.md`, it also writes `docs/critique-episode-N.html`. Critique pages keep the **same H1 title and chapter headings** as the article (heading order and timestamps are validated by `check_critiques.py`) but contain **no YouTube video links**: the H1 is plain text, chapter headings are plain headings, and there is no `منبع: تماشای ویدیوی اصلی` link. Each critique heading is readable at the matching anchor and links back to the article.
5. Article↔critique cross-links: on article pages the H1 and each chapter heading carry a `a.critique-link` magnifier icon (on the left of the title) linking to the critique page — the H1 to `critique-episode-N.html` and chapter `{i}` to `critique-episode-N.html#critique-{i}`. On critique pages the same headings carry a `a.critique-link.back` arrow linking back to `episode-N.html#ch-{i}`. The anchor ids pair up by chapter order (`ch-{i}` on the article, `critique-{i}` on the critique); the homepage episode cards also carry a critique icon.
6. Every page is fully RTL, `lang="fa"`, uses the Vazirmatn typeface, and shares a single stylesheet at `docs/assets/style.css`.
7. Page chrome is consistent across pages: the article's H1 links to the source video, the header subtitle links to the YouTube channel, the footer links to the channel and notes that the content is AI-generated and may contain errors, and a GitHub ribbon (`برو به گیت‌هاب`) in the top-right corner links to the repository. Episode pages end with a `منبع: تماشای ویدیوی اصلی` link; the homepage places the same link just before the episode listing.
...
- `scripts/build_site.py` owns **content and structure** — accurate markdown→HTML, correct chapter jump-links, the critique pages with their back-links, the page shell (header/nav/footer), and the `index.html` episode cards. Deterministic and reproducible, so it is the source of truth for the generated `.html` files.
