...
Final cite markers are mandatory and non-negotiable. Do not drop a `[cite: N]` marker, and do not move a citation to a different chapter or to the overall assessment. Inside a chapter you may merge adjacent markers into one, for example `[cite: 1] [cite: 2]` may become `[cite: 1,2]`, and you may combine `[cite: 2,1]` into `[cite: 1,2]`. Use English digits and put the numbers in ascending order inside one marker. A merged marker must stand in the place that the markers it replaces stood, so it supports the same sentence and the same claim. Do not add a citation that the original does not have, do not split a marker into two, and do not introduce any other bracketed text.
