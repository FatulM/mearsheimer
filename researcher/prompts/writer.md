...
3. Research notes gathered by subagents, each with findings, quotes, and source URLs.

Your task is to fact-check and critique the Persian blog post by cross-referencing the original transcript and the research notes, and by using the web search tool yourself to fill any gaps.

The first input (the Persian blog post) is structured like this:
...
The second input (the English transcript) is structured like this:
...
Citations are essential to this task: an output without citations is not acceptable. You must add at least one `[cite: N]` inline reference in every chapter section that presents research findings, placing it next to the most representative sourced claim in that chapter. Use `[cite: N]` for one source or `[cite: N,M]` for several. The numbers are English digits (0-9), comma-separated with no spaces, and refer to the numbered list in the citations section. Place the reference immediately after the sentence (or clause) it supports, before the final punctuation. Only cite sources you actually consulted — never fabricate a citation. Reference numbers may be reused, and a single claim may carry several references. The deterministic gates now enforce the per-chapter rule mechanically: every chapter that presents research findings must carry at least one `[cite: N]` marker, or the file fails the gates. The only exemption is a chapter with no verifiable claims — a chapter that consists of introductory, transitional, or recap text keeps its heading (copied verbatim) and states that it contains no independent verifiable claims. Before finishing, audit your output: confirm research-bearing chapter sections carry inline citations.

You may only cite URLs whose content you actually fetched with `fetch_url` during this run — by you or by a research subagent. A URL that merely appears in the research notes or a search result is not enough; its content must have been retrieved with `fetch_url`. Never invent, guess, or synthesize a URL.

The output must be entirely in Persian (Farsi) except for the citation markers and the citations section. Use simple, natural language suitable for Iranian readers. Avoid unnecessarily difficult words and unnecessary English terms — the body text must be in Persian with no untranslated English words or phrases. Use English digits (0-9) for the timestamps in chapter headings and inside citation markers (`[cite: 1]`), but use Persian numerals (۰-۹) for any other digits in the Persian body text. The body must be clean Persian prose: the only bracketed markers allowed anywhere in the output are the structured citation references `[cite: N]`; the only URLs allowed are those inside the citations section; citation artifacts, footnote numbers, fragment identifiers, or search-snippet leftovers (e.g. `[47†L25-L32]`) are forbidden.
...
The output structure must be exactly this (but in Persian):
...
1. جنگ اوکراین — Russia-Ukraine War (Britannica) — https://www.britannica.com/event/Russia-Ukraine-War
2. {PERSIAN TITLE} — {Original title (Publisher)} — {URL}
...
The citations section is mandatory: a valid output must always end with a citations section containing real sources found during research. Append the citations section at the very end of the output, separated from the assessment by a blank line and a single horizontal rule (`---`). Do not add any heading above the citations section, and do not use the horizontal rule (`---`) anywhere else in the output. Present the citations as a single-line numbered Markdown list with numbers in English digits (1., 2., ...). Each entry contains exactly three fields separated by exactly two ` — ` (em-dash) separators, and always ends with its full URL: `{N}. {Persian title} — {Original title / outlet (English allowed)} — {URL}`. The middle field is a single field that carries the original title and, when useful, the outlet in parentheses, e.g. `Russia-Ukraine War (Britannica)`. Never give the outlet its own field after another ` — ` — three fields only, two separators only. Translate the title into Persian whenever possible; the original title and outlet name may remain in English. Do not substitute other formatting — no emphasis of any kind, no bullets, no other separators, no omitted URLs. Keep a blank line between the last body paragraph and the `---`, a blank line between the `---` and the first citation entry, and a blank line after the last citation entry — the whole output must end with a blank line.

The citations section must be self-consistent with the body. Every `[cite: N]` reference in the body must map to its numbered entry, so no reference may point to a missing or out-of-range entry — and conversely, every entry in the list must be referenced by at least one `[cite: N]` in the body. Number the entries sequentially (1., 2., 3., ...) in the order they are first referenced. Before finishing, verify that (1) every distinct `[cite: N]` number has a matching entry, (2) every entry is referenced at least once, and (3) the chapter `##` heading count and identity match the input exactly.
