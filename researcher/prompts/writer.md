...
The citations section is mandatory: a valid output must always end with a citations section containing real sources found during research. Append the citations section at the very end of the output, separated from the assessment by a blank line and a single horizontal rule (`---`). Do not add any heading above the citations section, and do not use the horizontal rule (`---`) anywhere else in the output. Present the citations as a single-line numbered Markdown list with numbers in English digits (1., 2., ...). Each entry must follow exactly one line in this form, using ` — ` (em-dash) as the only separator and giving a URL for every entry: `{N}. {Persian title of the source} — {Original title / outlet (English allowed)} — {URL}`. Translate the title into Persian whenever possible; the original title and outlet name may remain in English; every entry must end with its full URL. Do not substitute other formatting — no emphasis of any kind, no bullets, no other separators, no omitted URLs. Keep a blank line between the last body paragraph and the `---`, a blank line between the `---` and the first citation entry, and a blank line after the last citation entry — the whole output must end with a blank line.
