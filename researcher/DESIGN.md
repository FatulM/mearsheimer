...
After the single simplification pass, a reviewer/fixer loop runs. The reviewer (`LLM_MODEL_AGENT_REVIEWER`, no web tools) compares the original body with the simplified body and returns a JSON verdict: `{"ok": bool, "problems": [...], "revised": "<body>"}`. It repairs fidelity problems with minimal edits, never re-simplifying. Deterministic gates run before every reviewer pass and before the write, so structure and citation self-consistency are enforced alongside the model verdict. One gate compares the ordered `[cite: N]` marker sequence of the simplified body with the original and fails on any added, removed, renumbered, or reordered marker, so citation loss is caught mechanically even when the citations stay self-consistent. The loop accepts only when the reviewer reports `ok` and the deterministic gates are clean.
